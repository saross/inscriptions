...
# -*- coding: utf-8 -*-
"""
03-ppc.py --- Step 3 of the H3a confirmatory run: the posterior-predictive
check suite.

Runs all TEN checks pinned in the design artefact (planning/
h3a-design-artefact-2026-06-04.md §2-§5) against the PRIMARY posterior, with
the two-tier severity adjudication (§6). A CRITICAL trigger is reported and
HALTs (no improvised model revision); a MINOR trigger is logged as a caveat.

The ten checks:
  1  proportion of zeros            (§3, bound: pp prop-zeros <= 0.02)
  2  mean count                     (§3, within +-10% of observed)
  3  SD of counts                   (§3, within +-25% of observed)
  4  95th percentile                (§3, obs<=tail_count_bound AND pp within +-30%)
  5  mean-variance ratio            (§3, pp ratio within [0.5x, 2x] of observed)
  6  Bayesian p-values for #2-#5    (§3, 0.05 <= p_B <= 0.95)
  7  residual-vs-fitted slope       (§4, |slope| < 0.10)
  8  residual-vs-within-logpop slope(§4, |slope| < 0.10)
  9  province residual dispersion   (§4, per-province mean|r| in [0.5x,2x] overall; n>=5)
  10 posterior-predictive Moran's I (§5, obs in 5th-95th pct of pp distribution)
...
Outputs
-------
runs/2026-06-04-h3a-confirmatory/outputs/ppc-results.json
...
def severity(value: float, bound_magnitude: float, in_band: bool,
    ...
    """Two-tier severity (design artefact §6).

    `value` is the absolute distance outside the bound (0 if in band).
    Critical: outside by > 2x bound magnitude. Minor: outside by <= 1.5x.
    Grey zone [1.5x, 2x]: adjudicated -> flagged 'grey'.
    For proportion-of-zeros, an ABSOLUTE 0.10 critical cutoff.
    """
    if in_band:
        return "pass"
    if abs_zeros:
...
def posterior_predict(idata: az.InferenceData, n_draws: int,
    ...
    Uses log_mu and dispersion from random posterior draws.
    """
...
def main() -> int:
    ...
    in5 = 0.5 <= ratio5 <= 2.0
    # distance outside multiplicative band, expressed in bound magnitudes
    if in5:
        dist5 = 0.0
    elif ratio5 > 2.0:
        dist5 = ratio5 - 2.0
    ...
    checks.append({
        ...
        "severity": severity(dist5, 1.0, in5),  # band half-width ~1 on ratio
    ...
    w.transform = "r"
    # observed Moran's I on the posterior-mean residual vector (H3c(ii)):
    moran_obs = Moran(r, w, permutations=0, two_tailed=False).I
    # posterior-predictive distribution of Moran's I: for each y_rep draw,
    # compute Pearson residual of y_rep against its own mu, then Moran's I.
    post = idata.posterior
    ...
    # distance outside the 5-95 band, scaled by band half-width
