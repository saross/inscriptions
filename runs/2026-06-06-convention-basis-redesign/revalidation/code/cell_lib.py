...
def build_tier_basis(design: dict[str, Any], env: Envelope) -> np.ndarray:
    ...
    Decision-38 extension (2026-06-06, this revalidation copy only): when the
    design carries a precomputed empirical basis under ``tier_basis_empirical``
    (the empire-frame 3 x N_BINS array built by
    ``runs/2026-06-06-convention-basis-redesign/code/02-build-empirical-basis.py``),
    return it directly — this is the empirical calendar-slab basis the recovery
    re-validation generates synthetics from and fits against. Backward-compatible:
    designs without that key fall through to the original
    ``template_intervals_by_tier`` construction below, byte-identical to the
    2026-05-22 build_tier_basis (the lodged 2026-05-26 grid is unaffected — which
    is why this change lives in a copy, not the frozen harness).

    Original path: row k is the interval-width-normalised uniform-mass vector over
    tier k's template-interval dictionary, with within-tier intervals
    equally-weighted and the row renormalised to sum to 1.
    """
    if "tier_basis_empirical" in design:
        basis = np.asarray(design["tier_basis_empirical"], dtype=float)
        if basis.ndim != 2 or basis.shape[1] != env.n_bins:
            raise ValueError(
                f"tier_basis_empirical has shape {basis.shape}; expected "
                f"(n_tiers, {env.n_bins})."
            )
        row_sums = basis.sum(axis=1)
        if not np.allclose(row_sums, 1.0, atol=1e-8):
            raise ValueError(
                f"tier_basis_empirical rows must each sum to 1; got {row_sums}."
            )
        return basis
