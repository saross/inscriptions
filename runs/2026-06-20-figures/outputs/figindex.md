...
| F9 | `fig09-variance-partition` | 1-col | **Clean three-way temporal partition** (38/29/33) + four component magnitudes | `h5-decomposition.json` + `temporal-three-way-split.json` | `fig09_variance_partition.py` |
...
- `temporal-three-way-split.json` (`compute_temporal_split.py`): the exact
  ...
  *standalone* common share reproduces the published 54 % (guard-checked); the
  three tiers are **anti-correlated** (standalone shares sum to 131 %), so they do
  not partition cleanly. **DECIDED (Shawn 2026-06-20): report the clean
  covariance-attributed partition** — **empire-common 38 % / province 29 % /
  city-unique 33 %** (Latin 37/30/33; mean-aggregated, sums to 100 %) — with the
  54 % standalone share as a footnote. Applied in F9 and
  `reports/key-findings-summary-2026-06-20.md`; the old indicative 54/24/22 is
  superseded.
