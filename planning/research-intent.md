...
A complementary framing applies at the urban scale: inscriptions function as **infrastructure for information** (Hanson 2021, pp. 139–140, 147) and scale **sublinearly** with urban population. Hanson (2021, Table 7.3) reports **β = 0.672 mean (95 % CI [0.588, 0.756]) / 0.654 median [0.514, 0.774]** for direct inscription-count-vs-population regression (OLS log-log on eight equally-sized population bins, n = 554 Empire cities with ≥ 1 inscription, EDCS corpus, Rome excluded as an extreme outlier). Supporting evidence comes from Hanson, Ortman & Lobo (2017) β = 0.686 (SE 0.078) for functional diversity vs population, Carleton et al. (2025) β ≈ 0.3–0.5 for elite-honorific inscription proxies (Bayesian), and Ross (2024) archived preliminary analysis β ∈ [0.47, 0.68] across OLS log-log and Negative Binomial regressions on LIRE v3.0 (n = 816 cities). Sublinearity is robust across datasets, regression families, and research groups. Per-capita inscription rates therefore *decrease* with city size — behaviour consistent with communication-infrastructure saturation (compare Roman roads and piped water, also sublinear in Hanson, Ortman, Bettencourt & Mazur 2019) rather than super-linear socio-economic output (compare coinage, pottery, housing: β > 1 per Ortman, Lobo et al. 2024 in Roman Britain). The multi-factor decomposition identifies *what drives* inscription production; the information-infrastructure framing explains *how production scales with settlement size*. Both inform the paper's claims.
