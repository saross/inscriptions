...
a suggestive but artefact-contaminated histogram.
...
1. **The reachability map (the N-floor).** A validated rule — "subset-specific
   de-fogging is trustworthy down to N ≈ ___, degrading above α ≈ 0.70, with
   overconfident bands for sharply-peaked signals" — is a *reusable deliverable*,
   not a one-off result. It tells every future researcher with a subcorpus
   whether the method is in their toolkit *before* they invest in it. (Delivered
   by the small-N reachability study.)
2. **At least one substantive subset demonstration** where it works — a large
   ...
   sits at the current validated floor) — showing a de-fogged trajectory that the
   raw histogram could not give.
...
- **Sufficient N.** Below the reachability floor (to be measured), a subset cannot
  be reliably de-fogged on its own; fall-backs (pooled convention, descriptive,
  the §5 hierarchical model) apply.
