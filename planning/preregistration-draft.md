...
- **Bayesian NBR for H3a:** `log(E[inscriptions_city]) = α_0 + α_province + β · log(population_city) + ε`, fitted with weakly informative priors, negative-binomial family, log link, provincial random intercepts. Library: `pymc` or `brms`-via-R (TBD; Shawn to confirm). Bayesian R² reported per Gelman, Goodrich, Gabry & Vehtari (2019). Full posterior distributions retained for downstream residual analysis.
...
- Bayesian-NBR software choice: `pymc` (stays within Python venv) vs `brms`-via-R (mature ecosystem; matches Carleton et al. 2025 approach).
