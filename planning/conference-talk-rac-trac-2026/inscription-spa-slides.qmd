...
- **Empire**: reachable at n = 50,000 *(only n tested at empire scale)*
- **Province / urban-area**: min n ≈ **1,600** *(range 1,400 – 1,950 across nulls)*
...
WHY "EMPIRE n = 50,000 ONLY"? The empire was simulated at a SINGLE
n-grid point (n = 50,000), not across a range. The actual filtered
empire corpus is N = 180,609 inscriptions, so testing at smaller n
values doesn't correspond to any real-world empire-scale analysis we'd
plausibly run. The empire-level result reads "yes, reachable at this
scale" — but doesn't pin a lower bound on what would also be
reachable. Province and urban-area, by contrast, were tested across
the full n grid (100 → 25,000 for province; 25 → 2,500 for
urban-area), because real provincial / urban-area corpora vary
substantially in size. That's where the "min n ≈ 1,600" floor comes
from — it's the smallest n at which the province / urban-area
simulations achieve ≥ 80 % detection on the binding bracket.

WHY "1,600" AND NOT "1,549" OR SOMETHING MORE PRECISE? Because the
actual measured thresholds vary across the four null-model
specifications: province values are {1,385, 1,618, 1,869, 1,938};
urban-area values are {1,409, 1,549, 1,854, 1,923}. The two levels'
ranges essentially overlap, justifying a single headline figure. The
rounded "≈ 1,600" sits inside both measured ranges (slightly more
conservative than the median across nulls) and avoids implying false
precision. If asked: the exact median across all 8 measured cells is
~ 1,700; we round down to 1,600 as a slightly-more-conservative
headline.
...
::: {.columns}
::: {.column width="60%"}
![Detection rate by N (rows) × effect bracket (columns). Grey cells = combination not in the simulation grid for that level.](figures/fig-reachability-historian.png){fig-align="center" width="100%"}
:::
::: {.column width="40%"}
**Practical reads**:
...
<!--
SPEAKER NOTES — slide 3b (≈ 90 s):

WHAT THIS SLIDE DOES: extends slide 3a's binding-bracket headline
(min N ≈ 1,600 for the 50 %-over-50 y bracket at province / urban-area
level — the actual measured values range 1,385 – 1,938 across the
four null-model variants; "1,600" is the rounded headline figure)
to the broader effect-detection envelope. For a historian asking "can
my subset of N inscriptions support a confirmatory deviation-detection
claim?", this reads off the answer.

THE PRACTICAL READS:
  - Below N ≈ 1,000: effectively unreachable for confirmatory
    deviation-detection at the preregistered false-positive rate.
    The inscription series can still support descriptive shape
    comparison (which we do on slide 4) and contribute leverage to
    cross-sectional regressions (slide 6b).
  - At N ≈ 2,500: the binding 50 %-over-50 y bracket becomes
    reliably reachable (detection rate ≥ 0.97). This is where empire-
    wide subsets like collegia (~ 3,000), military-diploma inscriptions
    (similar order), or specific religious-cult subsets start being
    analytically interesting.
  - The 20 %-over-25 y bracket (small, narrow events) is below the
    method's resolving power at any practical N we tested. This is a
    property of inscription-SPA inference broadly, not specific to our
    method.

WHY THIS MATTERS FOR THE FIELD: dates-as-data work on inscription
corpora has historically been opportunistic — researchers test
whatever subset is available, against whatever pattern catches the
eye, without prior calibration. The Phase 1 reachability map is a
pre-analysis discipline: it tells you before you run an analysis
what claims your subset can credibly support, and where you should
drop to descriptive / cross-sectional reasoning instead.

WORKED EXAMPLE for Q&A: a hypothetical collegia / guild inscription
subset (~ 3,000 inscriptions empire-wide) reads off as REACHABLE for
the binding bracket at empire scale; PARTIAL at provincial scale;
OUT OF SCOPE at single-city scale. The historian knows BEFORE running
the analysis what claims the subset can support.

SCOPE caveats to soft-pedal aloud, in speaker notes for reference:
  - This is for ONE specific method (the preregistered permutation-
    envelope test). Bayesian aoristic methods (Crema 2025, baorista)
    may have different — possibly more permissive — reachability
    profiles; a parallel analysis is on the post-talk to-do list.
  - The current grid covers three effect-size brackets. Tier 2 of the
    follow-up work fills in finer brackets; Tier 3 produces per-subset
    reachability tables using each subset's own aoristic-width
    distribution. Both are committed to in the paper's methodology-
    supplement section.
-->
