...
**Post-draft update — two stand-in cross-model statistical reviews (2026-05-17), and consequent prereg edits.** While the pack was being finalised, the same draft was sent to ChatGPT 5.5 and Gemini 3 Pro for stand-in statistical reviews (`planning/GPT55-statistical-review.md`, `planning/gemini-statistical-review.md`) — a hedge against the timing risk of Martin's reply arriving after Adela's Friday 2026-05-22 conference presentation. Two cross-model-agreement items (replicate count → 100; Wasserstein-1 supplementary shape metric) and five single-model items (an aoristic-Monte-Carlo sensitivity; a posterior-predictive spatial-autocorrelation PPC; a population- / inscription-weighted variance-fraction sensitivity; a three-case interpretive guardrail for H3c(ii) Moran's I; severity levels on PPC triggers) have been **incorporated into the preregistration draft as Decisions 27–32** and into the relevant prereg sections. The pack reflects the post-Decision-27-to-32 state; each stand-in-derived change is flagged inline with `[stand-in update]` for traceability. Your input on any of these is genuinely sought — **if your response arrives by Tuesday 2026-05-19 it can be folded into the prereg before OSF lodgement**; otherwise Decisions 27–32 lodge as currently written and any reversal you recommend is filed as a post-lodgement OSF amendment, per the §7 contingency rules. **Background and exact substance** of each stand-in-derived change is recorded in the decision log (Decisions 27–32 at `planning/decision-log.md`) and in the changelog (`planning/preregistration-changelog.md`, the "2026-05-17 (later) — Stand-in cross-model statistical review" section).

How you engage is up to you: marginal notes on a printout, an email reply keyed by question number, a call, a written memo — whichever fits your time. Even partial input on one or two of the primary questions is genuinely useful; this pack errs on the side of completeness because OSF lodgement is the next step (whether or not your full input is in hand by Tuesday), so getting it wrong has more consequence than getting it long.

---

## Executive summary

**Why this preregistration matters.** Latin inscriptions from the Roman Empire are one of the most abundant systematic datasets from antiquity — ~ 181 k geo-temporally locatable inscriptions in LIRE v3.0 alone — and have long been treated by ancient historians, archaeologists, and (more recently) economic historians as a proxy for demography, urbanism, and economic activity. But inscription production responds to several drivers simultaneously: urban population, urban scale and function (capitals over-produce, frontier garrisons over-produce), an empire-wide "epigraphic habit" rise-and-fall in the intensity of inscribing across the Roman period, systematic between-province differences in epigraphic culture, and editorial-template encoding artefacts in the source corpus. **The question this preregistration tests is whether a demographic signal is recoverable from the temporal and spatial distribution of inscription production**, at three scales (empire, province, urban area), once those competing drivers and the source-data artefacts are accounted for. The headline confirmatory quantity is the within-province population-attributable variance fraction at city level; the Hanson (2021) sublinear scaling (β ≈ 0.67) and his secondary residual findings (provincial capitals over-produce; residuals are spatially clustered) are tested as replications on a corrected, preregistered Bayesian pipeline. A clean answer — supported, evidence against, or inconclusive — informs what inscription counts can and cannot be read as in this corner of Roman history, which in turn constrains how confidently the substantial existing literature on Roman urbanism and demography can lean on inscription-density data.

**The project.** A three-phase analysis: Phase 1 (a Monte Carlo power simulation, complete) establishes minimum sample sizes for permutation-envelope deviation-detection at each subset level; Phase 2 (a Bayesian deconvolution-mixture model with a recovery-simulation validation) separates the corpus-level temporal SPA — "summed probability analysis," a kernel-density-style empirical curve over time — into an editorial-encoding-artefact component and a genuine ancient-pattern component; Phase 3 (a Bayesian within-between Mundlak negative-binomial regression of city-level inscription counts on log-population, plus Hanson-replication residual analyses) carries the substantive estimation. The corpus is LIRE v3.0 (~ 181 k inscriptions, ~ 815 cities with matched Hanson 2016 population estimates).
...
*Q1, Q2, Q3, Q4, Q5, and Q8 reflect prereg edits made on the basis of two stand-in cross-model statistical reviews (ChatGPT 5.5 + Gemini 3 Pro, both stored in `planning/`) — incorporated into the preregistration draft as Decisions 27–32. Each such update is marked `[stand-in update]` in-text with the source Decision number. Reversal of any of Decisions 27–32 is filed as a post-lodgement OSF amendment if your review recommends it. See the acknowledgement at the bottom of "How to read this pack" for the rationale and scope.*
...
- **(c)** Is the decision to treat aoristic uncertainty as upstream (one SPA → one fit) defensible at corpus level, or do you anticipate that the posterior on α would shift materially under per-inscription-latent-date propagation? **Incorporated [stand-in update — Decision 28]:** an **aoristic-Monte-Carlo supplementary fit** is preregistered — re-run the primary multinomial mixture on N_MC ∈ [20, 50] independently-sampled aoristic SPA realisations (each from a different per-inscription latent-date draw within `[nb_i, na_i]`), report the cross-realisation posterior on α as a sensitivity on the upstream-aoristic assumption, with a divergence flag at 1.5× the primary posterior's 95 % CI width. Compute cost is modest (a few extra mixture fits). Gemini's defence of the current upstream-aoristic choice rests on a Law-of-Large-Numbers argument ("per-inscription uniform assignment washes out variance at the 5-year bin level"), which is plausible at N_eff ~ 10^5 but not airtight where wide template intervals dominate; GPT5.5 proposed this MC sensitivity as a direct test. Does the aoristic-MC sensitivity look right as preregistered, or would you prefer something more elaborate (e.g. propagating latent dates into the likelihood directly), or no sensitivity at all? Reversal possible via OSF amendment.
...
- **(b)** Replicate count. The prereg-binding floor was bumped from ≥ 50 to **≥ 100 [stand-in update — Decision 27]** on the basis of both stand-in reviewers independently flagging 50 as thin (a true 90 % coverage cell at n = 50 has a 95 % Wilson interval ~ [0.79, 0.96], so the per-cell pass / fail boundary is brittle). Both reviewers prefer 100; GPT5.5 floats a two-stage variant (50 across full grid → 200 at boundary cells) if compute is tight. Is 100 defensible, or do you want us to push to 200 for the smaller final grid? Cost is roughly linear in replicates × cells; expect O(50–100) cells. Reversible via OSF amendment.
- **(c)** Shape-recovery metric. Pearson r ≥ 0.95 is retained as the **headline continuity metric (binding rule unchanged)**; both stand-in reviewers independently flagged it as too forgiving for localised mass-redistribution failures, so a **distribution-sensitive supplementary metric has been added [stand-in update — Decision 27]**: **Wasserstein-1 (Earth Mover's distance)**, with Jensen-Shannon divergence and integrated absolute error as alternatives considered but not adopted. Both Wasserstein-1 and JSD are the gold-standard distribution-comparison metrics for compositional data; both reviewers' concern is that Pearson r can stay high while bin-localised mass is mis-allocated. **Preregistered reporting:** Pearson r remains the binding rule (≥ 0.95 in ≥ 90 % of cells); Wasserstein-1 is reported alongside descriptively per cell, with a flagging threshold pinned in the design artefact. **Two questions for you:** (i) is Wasserstein-1 the right primary supplementary, or would you reach for JSD / IAE instead? (ii) should the supplementary metric be promoted to part of the binding decision rule, or kept descriptive? Reversal possible via OSF amendment.
...
- **(c)** Is the **supplementary "posterior distribution of I_s per k"** an adequate way to surface the posterior uncertainty that the confirmatory rule hides? **Three-case interpretive guardrail [stand-in update — Decision 31]** (from GPT5.5; now preregistered in §3 alongside the confirmatory rule):
   - **Case 1 — strong support:** posterior-mean Moran's I is permutation-significant at p < 0.05, AND the posterior distribution of `I_s` is mostly positive (e.g. ≥ 95 % of draws above 0).
   - **Case 2 — permutation-significant but posterior-sensitive:** posterior-mean Moran's I is permutation-significant, BUT the 95 % posterior interval of `I_s` crosses zero. Reported as "spatial clustering significant on the posterior-mean residual surface but sensitive to posterior uncertainty" — not described as a clean replication of Hanson 2021's finding.
   - **Case 3 — confirmatory rule passes but no substantive support:** posterior-mean Moran's I is permutation-significant, but the posterior distribution of `I_s` is centred near zero. Reported as "the confirmatory rule passes but the result does not survive posterior-uncertainty diagnostics" — H3c(ii) **not claimed as substantively supported**.

   The decision rule itself (Moran's I > 0 at p < 0.05 in ≥ 2 of {k = 5, 8, 10} on posterior-mean residuals) is *unchanged*; the guardrail governs only the interpretive language used in the paper. Thresholds (≥ 95 % of draws for Case 1; < 50 % for Case 3) are committed in the prereg. Does this look right, or would you prefer different posterior-distribution thresholds (e.g. ≥ 90 % rather than ≥ 95 % of draws above 0 for Case 1)? Reversible via OSF amendment.
...
- **(a)** Is the **category set complete**? **Incorporated [stand-in update — Decision 29]:** an **8th category — posterior-predictive spatial-autocorrelation on H3a residuals** (predict `y_pred` under the fitted H3a posterior; compute Moran's I on the resulting Pearson-residual surface; trigger if the model routinely *fails* to generate residual spatial structure of the observed magnitude, which would mean H3c(ii) is testing something the model can't produce *a priori*). Adopted from GPT5.5's recommendation; directly addresses our own Q4(d) ask about whether an H3c-specific PPC is needed. **Still-open candidates** (not yet adopted; flagged for your view): autocorrelation in standardised residuals if ordered by population or province; posterior-predictive *log-likelihood replicate distribution* (Bayesian-p style global fit check); GPT5.5 also suggested **top-k inscription share**, **maximum / 99th percentile city count**, **distribution of log-counts**, and **posterior-predictive distribution of province-level totals** — heavy-tail-appropriate alternatives to the current mean / std formulation. Anything else missing? Anything you'd remove?
...
- **(c)** Is the **failure response defensible**? **Incorporated refinement [stand-in update — Decision 30]:** a two-tier severity scheme replaces the previous uniform "any tripped trigger" rule. GPT5.5's concern was that a mild tail discrepancy could force a formal model revision and OSF amendment even when the main model is adequate. The preregistered two-level scheme: **critical** triggers (significantly tripped — e.g. PP mean off by > 2× the design-artefact bound; residual-vs-fitted slope > 2× threshold) initiate model revision, original-and-revised reporting, and OSF amendment; **minor** triggers (marginally tripped — e.g. PP mean tripped but within ≤ 1.5× the bound) are reported as caveats in the paper without forcing model revision or amendment. The specific 2× / 1.5× cutoffs are straw values, pinned per PPC category in the design artefact. Does the severity-level scheme look right, or do you prefer a three-tier scheme, or different cutoffs (e.g. critical at > 3×; minor at > 1× ≤ 2×)? Reversal possible via OSF amendment.
- **(d)** Is there a **further PPC-trigger category that should be H3c-specific**, beyond the posterior-predictive spatial-autocorrelation check proposed in (a) — e.g. a check on the *posterior-mean residual surface* before Moran's I is run, to ensure the residuals aren't already smooth enough to bias the spatial test?
...
- **(e)** **Variance weighting [stand-in update — Decision 32].** The `f_within` denominator `Var(log E[inscriptions_c])` is computed *unweighted* across cities for the binding primary — answering "what share of city-to-city systematic variation does within-province population explain?" GPT5.5 flagged this as a defensible-but-arbitrary choice that a reviewer may ask about, and recommended a **population-weighted or inscription-weighted variance-fraction sensitivity** as a §5 exploratory. **Preregistered §5 sensitivity:** report `f_within` under unweighted (primary, binding confirmatory), population-weighted (`w_c = population_c`), and inscription-weighted (`w_c = y_c`) variance denominators; the three together answer different substantive questions and the spread (flagged as material if exceeding half the primary 95 % CI width) is itself informative. Does this look like a sensible exploratory addition, or are there hidden traps (e.g. inscription-weighting effectively gives the largest cities disproportionate leverage on the variance-fraction estimate)? Reversal possible via OSF amendment.
...
- **(b)** **Would you like to suggest specific pinned values now**, in writing, that we then propagate into the artefacts? Areas where we are most uncertain and most want your input:
  ...
  - **Recovery-grid replicate count.** Prereg-binding floor bumped from ≥ 50 to **≥ 100 [stand-in update — Decision 27]** on the basis of independent cross-model review by ChatGPT 5.5 and Gemini 3 Pro (both flagged 50 as thin for the per-cell Wilson interval on coverage). Design-artefact default is 100. Push further to 200 if you'd prefer, or use a two-stage variant (50 across full grid → 200 at boundary cells).
...
**Why aoristic uncertainty is upstream not per-inscription.** We considered per-inscription latent-date sampling propagated into the likelihood (each inscription with date range `[nb_i, na_i]` would have a latent `t_i ~ Uniform(nb_i, na_i)` in the model). We rejected this for the primary because (i) it dramatically expands the parameter space (N_eff ~ 10^5 latent dates); (ii) we expect the aoristic-uncertainty effect on α to be small at corpus scale (the mixture is a *shape* model on binned mass, and the per-year uniform aoristic deterministically maps interval data to bin mass — re-sampling within each interval would average to the same expectation). Gemini supports this rationale with an explicit Law-of-Large-Numbers argument: "by the Law of Large Numbers, the per-inscription uniform assignment washes out variance at the 5-year bin level."

**Aoristic-MC supplementary [stand-in update — Decision 28].** GPT5.5 proposed an intermediate alternative: rather than propagating latent dates into the likelihood, run the mixture on **N_MC independently-sampled aoristic SPA realisations** (each from a different per-inscription latent-date draw), and report the cross-realisation posterior on α as a sensitivity. This directly tests whether the LLN-based defence holds in practice at our N_eff, and is much cheaper than full latent-date propagation (each realisation is one extra mixture fit; N_MC ∈ [20, 50] is preregistered, with specific N_MC value pinned in the design artefact). Preregistered scope: N_MC fits in addition to the primary single-SPA fit; results reported as the cross-realisation posterior of α (median and percentile range) alongside the primary posterior; **divergence flag preregistered** — if the cross-realisation 95 % range on α exceeds 1.5× the primary posterior's 95 % CI width, reported as a material limitation in the paper (no amendment trigger; preregistered sensitivity). **Incorporated in the prereg as Decision 28; reversal possible via OSF amendment** — see Q1(c).
...
| Replicates per cell | **≥ 100** (prereg-binding floor) [stand-in update — Decision 27, bumped from ≥ 50 on cross-model review] | Default 100; two-stage variant (50 across full grid → 200 at boundary cells) acceptable per the design artefact |
...
**Supplementary shape metric [stand-in update — Decision 27].** A distribution-sensitive supplementary metric — **Wasserstein-1 (Earth Mover's distance)** between posterior-median and true genuine SPA — is reported alongside Pearson r per cell, with a flagging threshold pinned in the design artefact. JSD and integrated absolute error were considered as alternatives but not adopted. Added on the basis of both stand-in reviewers independently flagging Pearson r as too forgiving for localised mass-redistribution failures. The supplementary metric is *descriptive*, not part of the binding rule; whether it should be promoted to a co-binding rule (e.g. Wasserstein-1 ≤ pinned-threshold in ≥ 90 % of cells) is a question for Martin. Reversal via OSF amendment.
...
**Three-case interpretive guardrail [stand-in update — Decision 31].** Preregistered in §3 alongside the confirmatory rule:

| Case | Posterior-mean Moran's I (permutation) | Posterior distribution of `I_s` | Reported as |
|------|----------------------------------------|----------------------------------|-------------|
| 1 | Significant at p < 0.05 in ≥ 2 of 3 k | ≥ 95 % of draws > 0 | Strong support — clean replication of Hanson 2021. |
| 2 | Significant at p < 0.05 in ≥ 2 of 3 k | 95 % interval crosses zero | Permutation-significant on posterior-mean residual surface, but sensitive to posterior uncertainty. Not described as clean replication. |
| 3 | Significant at p < 0.05 in ≥ 2 of 3 k | Centred near zero (< 50 % of draws > 0) | Confirmatory rule passes but does not survive posterior-uncertainty diagnostics. H3c(ii) **not** claimed as substantively supported. |

The decision rule itself is unchanged; the guardrail governs only the interpretive language used in the paper. Adopted from GPT5.5's stand-in review; thresholds (≥ 95 % of draws for Case 1, < 50 % for Case 3) preregistered in Decision 31. Reversal via OSF amendment.
...
| **8** | **Posterior-predictive spatial autocorrelation on H3a residuals [stand-in update — Decision 29]** | **Moran's I on `y_pred` residual surfaces vs observed I; trigger if observed I lies outside design-artefact-pinned range of the posterior-predictive I distribution (default 5th–95th percentile)** | **Design-artefact-pinned bounds; severity tiers per Decision 30** |
...
**Severity-level refinement [stand-in update — Decision 30].** GPT5.5 flagged the uniform "any trigger" rule as too aggressive: a mild tail discrepancy could force a formal revision and OSF amendment even when the main model is adequate. Preregistered two-level scheme:

| Severity | Trigger condition (relative to design-artefact bound) | Response |
|----------|--------------------------------------------------------|----------|
| **Critical** | Significantly tripped (e.g. > 2× the bound; or sign-flipped) | Model revision + OSF amendment + original-and-revised reporting (current uniform rule). |
| **Minor** | Marginally tripped (e.g. > 1× but ≤ 1.5× the bound) | Reported as a caveat in the paper; no model revision required; no OSF amendment. |

The 2× / 1.5× cutoffs are straw values; design-artefact-pinned. The refinement *preserves* the previous uniform rule's protection (critical failures still trigger amendment) while preventing minor failures from forcing model revision. Adopted from GPT5.5's stand-in review; reversal possible via OSF amendment.
...
**Weighting sensitivity [stand-in update — Decision 32].** The denominator is *unweighted* across cities for the binding primary. GPT5.5 flagged this as defensible but arbitrary — it answers "what share of *city-to-city systematic variation* does within-province population explain?" not "what share of *inscription-weighted variation*?" §5 preregistered exploratory: report `f_within` under three weighting choices alongside the unweighted primary:

1. **Unweighted** (primary; binding confirmatory): each city contributes equally to the variance computation.
2. **Population-weighted:** each city weighted by `population_c`, answering "what share of population-weighted log-count variance does within-province population explain?"
3. **Inscription-weighted:** each city weighted by `y_c`, answering "what share of inscription-weighted log-count variance does within-province population explain?"

The three together give a sensitivity envelope on `f_within`; the spread between them is itself substantively informative (large spread → the estimand is sensitive to whose variation we're partitioning).
...
| H2.1 (recovery simulation) | Genuine-shape recovery | Posterior-median Pearson r ≥ 0.95 between recovered and true genuine SPA in ≥ 90 % of cells. **Supplementary [stand-in update — Decision 27]**: Wasserstein-1 between posterior-median and true genuine SPA reported per cell with design-artefact-pinned flagging threshold. |
