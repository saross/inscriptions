...
started: 2026-04-24
last-updated: 2026-06-04 (reachability floor measured N≈2000 + checkpointing fix; Stage-3 cross-grid adjudication → inscription PASS / letter FAIL → inscription-mass only; field-standard benign-divergence gate refinement → Grid A re-scored B=A=98.6%, flat-null limitation dissolved, backlog re-fit retired; α-diagnostic → Bland–Altman LoA, shape-conditioned; OSF Amendment 01 fully encoded + plain-language summary + internal-tooling cleanup, lodge-ready (Shawn lodging); statistical-grounds memo + 12-source bibliography verified + staged to Zotero; both compute hosts caught up to main @ 554b95a)
---
...
**FIRST NEXT-SESSION ACTION: harness re-aggregation so `grid-summariser`/`compare-grids` reproduce Grid A = 98.6 % in-pipeline.** The 98.6 % is verified by a direct re-score from the stored per-replicate R̂/ESS, but the committed harness still emits 91.9 % (old cell-summaries computed under the zero-tolerance gate). Re-aggregate both grids' cell-summaries under the new `fit.py` convergence gate (R̂/ESS-only; NO re-fit — read the per-rep `cell-fits/*/replicate_*-posterior.json` on sapphire), update `grid-summariser.py` (`GRID_A_HEADLINE_B` 0.919→0.986 + the regression assertion), re-run `compare-grids.py` → regenerate REPORTs/parquets/COMPARISON-REPORT. Then annotate **Decision 33** (`planning/decision-log.md`) + **Obs 67** (`docs/notes/working-notes.md`) with the 2026-06-04 update (both still cite 91.9 % as historical). Confirm the OSF amendment lodged (Shawn was lodging it at session close).
