...
started: 2026-04-24
last-updated: 2026-06-04 (harness re-aggregation: Grid A 98.6% now reproduced IN-PIPELINE — convergence gate centralised in cell_lib.convergence_pass, aggregate.py re-derives from stored R̂/ESS, new reaggregate.py; both grids re-aggregated on sapphire no-re-fit, regression check passes; Decision 33 + Obs 67 annotated 91.9%→historical; sapphire now @ main 4f96e47. Earlier same day: reachability floor measured N≈2000 + checkpointing fix; Stage-3 cross-grid adjudication → inscription PASS / letter FAIL → inscription-mass only; field-standard benign-divergence gate refinement; α-diagnostic → Bland–Altman LoA; OSF Amendment 01 fully encoded, lodge-ready (Shawn lodging); statistical-grounds memo + 12-source bibliography verified + staged to Zotero)
---
