...
Sublinear scaling of inscriptions with urban population is empirically robust: Hanson (2021, Table 7.3) reports **β = 0.672 mean (95 % CI [0.588, 0.756]) / 0.654 median [0.514, 0.774]** for direct inscription-count-vs-population regression (OLS log-log on eight equally-sized population bins, n = 554 Empire cities, EDCS corpus, Rome excluded as an extreme outlier); Hanson, Ortman & Lobo (2017) β = 0.686 (SE 0.078) for functional diversity vs population; Carleton et al. (2025) β ≈ 0.3–0.5 for elite-honorific inscription proxies (Bayesian); Ross (2024) archived preliminary analysis β ∈ [0.47, 0.68] across OLS log-log and Negative Binomial regressions on LIRE v3.0 (n = 816 cities). Per-capita inscription rates therefore *decrease* with city size.

The multi-factor complexity-markers framing above accommodates this pattern: if each complexity dimension saturates at scale (cultural transmission of the epigraphic habit reaches diminishing returns; competitive status-signalling weakens in larger markets; information networks saturate), sublinearity is expected. An **alternative interpretation** proposed by Hanson (2021, pp. 139–140, 147) treats inscriptions as **infrastructure for information** — analogous to Roman roads and piped water, which also scale sublinearly (β ≈ 0.5–0.67 per Hanson, Ortman, Bettencourt & Mazur 2019) rather than like super-linear socio-economic output (coinage, pottery, housing: β > 1 per Ortman, Lobo et al. 2024 in Roman Britain). Under that reading, inscriptions measure communication-infrastructure saturation rather than social complexity *per se*. **For the RAC-TRAC 2026 conference paper the complexity-markers framing is primary, with the information-infrastructure reading presented briefly as an alternative**; audience response will inform whether one, both, or a synthesis carries into the full journal submission.
...
- **H3c — urban-area residuals.** Cities that systematically over- or under-produce inscriptions relative to Hanson-predicted counts, mapped and tabulated. Outliers are interpretable as entry-points for the other complexity dimensions (economy, differentiation, translator, ideology) in follow-up work. Extends Hanson (2021, pp. 148–149) residual analysis to mixture-corrected data; Hanson's own findings provide directly testable patterns: **(i)** provincial capitals over-produce inscriptions relative to scaling expectation (mean residual 0.43 versus ~0.06 for *coloniae* / *municipia*); **(ii)** residuals are spatially clustered (Moran's I = 0.046, z = 4.571, p < 0.0001). If the mixture-corrected SPA residuals reproduce these patterns, Hanson 2021 is replicated on corrected data; if they differ, the divergence is itself a substantive finding. Hanson pools across time periods — the current paper can extend by **resolving residuals chronologically** (per-period over/under-production), a dimension Hanson's design could not address.
...
- **FS-D: Cultural translator via mixture α.** The `α` parameter from the deconvolution mixture is itself interpretable as a proxy for epigraphic-habit intensity — regions and periods with α → 1 rely heavily on conventional dating, which is itself a signal about inscription production practices. Novel; would require its own methodological paper. For the current paper, α serves as the primary translator sensitivity covariate on H3 (see Open Question 4 resolution); FS-D is the dedicated methodological development of α-as-translator-index as a stand-alone contribution. A future composite index could triangulate α with Carleton et al. (2025) provincial random effects (spatial), Hanson (2021) scaling residuals (city-level), and Glomb et al. (2022) Monte Carlo temporal baselines.
- **FS-E: Aeneas-partition follow-up** (see `wiki/planning/future-studies.md` FS-1). Uses Aeneas text-signal variance to partition LIRE and cross-check whether main-paper signal survives the editorially-anchored subset.
...
1. **Conference venue and deadline** — still TBC per `archive/beacons/backlog-2026-04-22.md` §7. Determines final scope and abstract submission timing.
2. **Effect-size targets — calibrated 2026-04-24.** Decision 5 brackets (50 % over ≥ 50 y; doubling over ≥ 25 y; 20 % over ≥ 25 y; Antonine-anchored) remain appropriate for H3b deviation-detection. For H3a variance-explained, new pre-specification: **R² ≥ 0.25 at urban-area level** (anchored on Hanson, Ortman & Lobo 2017 R² = 0.267 on the same Hanson 2016 population dataset, and on Carleton et al. 2018 PEWMA simulation r = 0.5 / R² = 0.25 practical detection floor); **R² ≥ 0.50 at province level** (upper end of Palmisano et al. 2021 empirical SPD × climate range, Pearson r up to 0.77). Cohen's brackets (via Kolar et al. 2023) as SPD-field fallback. **Bayesian R² (Gelman et al. 2019) recommended reporting framework** — first SPD paper to adopt. Simulation-based power analysis to be built adapting Carleton et al. 2018 PLOS ONE code (CC-BY). See `runs/2026-04-23-prior-art-scouts/synthesis.md` §2.
3. **Scaling-law framing — resolved 2026-04-24.** Four-way convergence (Hanson 2021; Carleton et al. 2025; Hanson, Ortman & Lobo 2017; Ross 2024 archived) establishes that Roman inscriptions scale **sublinearly** with urban population (β ∈ [0.3, 0.7]). Sublinearity means raw correlation does not inflate the population effect via scaling (the opposite of the Ortman super-linear default initially assumed). **Framing B adopted**: raw correlation primary; scaling-residual analysis retained as robustness / sensitivity. Informs the information-infrastructure framing in the theoretical frame section above. See `runs/2026-04-23-prior-art-scouts/synthesis.md` §1.
4. **Cultural-translator confound — strategy defined 2026-04-24.** Prior-art scout found no published work that operationalises epigraphic-habit variation as a continuous, joinable covariate at empire scale (MacMullen–Meyer–Woolf framework remains qualitative; Beltrán Lloris 2015 explicitly rejects quantification; Benefiel & Keesling 2024 Brill volume largely qualitative). This is itself a reportable literature gap. Sensitivity strategy: (a) **mixture α parameter** as primary translator proxy (already in H2); (b) triangulation with three adaptable approaches — **Carleton et al. (2025) provincial random effects** (spatial; code at [github.com/wccarleton/urbanscale](https://github.com/wccarleton/urbanscale), CC BY 4.0), **Hanson (2021) scaling residuals** (city-level), **Glomb et al. (2022) Monte Carlo temporal baseline** (temporal). See `runs/2026-04-23-prior-art-scouts/synthesis.md` §3.

## Provenance

Drafted 2026-04-23 at Shawn's request, as precondition for the OSF preregistration of the `min-thresholds → mixture validation → population correlation` programme. Multi-factor theoretical frame articulated by Shawn; identifiability-scope reasoning, future-work decomposition, and scaling-law confound flag contributed by main-thread Claude in critical-friend register. Subject to Shawn's edits before commit. See `wiki/ai-contributions.md` for the standing log of substantive AI intellectual contributions.
