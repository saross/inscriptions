...
**The probe.** Rather than fix-and-move-on, I treated "two by luck" as evidence of a population: if accidental glances keep turning up value-level errors, a systematic value-level pass should turn up more. Ran a comprehensive adversarial accuracy-audit workflow — seven cluster verifiers re-reading every checkable number against its primary artefact, each flagged mismatch independently re-verified, plus a cross-document consistency scan.

**The belief revision.** Confirmed, decisively: 17 corrections across 677 checked specifics, including a corpus-column typo (65→63) that the *targeted* uplift agent had already "fixed" in one file but missed in a second. The revision is general and worth carrying: a **coverage** audit and a **consistency** audit are categorically not an **accuracy** audit. "Verified" at the structural level (every obligation discharged; every banner correct) says nothing about whether the *values* match source. The two-by-luck errors were not bad luck; they were the visible tip of a class the structural sweeps were blind to by construction.

**A secondary confirmation of method.** The adversarial second stage overturned exactly one first-verifier flag (an "Antonine ~168" that was a faithful rounding of 167.5) — i.e., the find-then-independently-refute design did its job of suppressing a false correction before it entered the record. Find-only would have introduced an error while removing others.

**A minor disconfirmation.** Shawn's implicit hypothesis that *capitals* might be the verbose cities was disconfirmed on a quick descriptive check: per-inscription verbosity is uncorrelated with capital status, size, or output — idiosyncratic — which (pleasingly) strengthens the independent-channels reading rather than complicating it.
...
**The belief revision.** α is the **convention fraction** (1 − α = genuine). The collaborator summary had it inverted — "two-thirds genuine" should read "two-thirds convention." The whole *canonical* corpus (lodged prereg, model docstrings, OSF amendments) already defined it correctly; only the derived summary (and, embarrassingly, my own prior reflection) carried the flip. The revision generalises last session's lesson one level deeper: a value-level accuracy audit verifies *number-against-source* and is structurally blind to inverted *semantics* when the number reads plausibly under either label (0.68 is a fine "two-thirds" either way). Correcting meaning needs a different check than correcting values — a reality anchor, like Pompeii.

**The surprising fact (secondary), and a clean revision.** Testing whether a *better-dated* (narrow-interval) women subset could escape the convention problem, I expected: narrower interval ⇒ more precise ⇒ more genuine ⇒ lower α. The dry-pass showed the opposite — the ≤50-year band is the *most* convention-aligned (0.91 raw; fitted α 0.97). **Revision:** the editorial convention does not live at *wide* intervals; it lives at *round* ones (the F1_round 25-year slabs — AD 100–150, 150–200 — are 68% of the corpus and are themselves "narrow"). So "tidiness" of an interval is not evidence of genuineness; round-number tidiness is the signature of the artefact. The naive precision intuition had the mechanism backwards, and a cheap dry-pass caught it before any fits were wasted on the wrong axis.
...
**The probe.** Instead of answering from intent, I read the doc in a cold reader's reading order — the sections a fresh instance actually enters through: "Priority artefacts (read in this order if context is cold)" and "If context feels cold." Both pointed a newcomer at the *preregistration draft* and *H1 simulation thresholds* as the starting point and "current results" — the project's state from ~2026-05-17, before the entire analysis programme, the figures, the outline, and the reorg — and via `planning/…` / `docs/notes/…` paths the reorg had just dissolved. A grep confirmed the staleness was systematic, not a stray line.

**The belief revision.** The doc was *not* self-sufficient, and the reason was a hidden equivocation in my own claim. "I updated the continuity doc" was true of the parts I was *editing in* (the frontmatter beacon, the remaining-work inventory) and false of the parts a cold reader *enters through* (the orientation / reading-list sections), which I had never touched. The two are different artefacts wearing one filename. Worse, I had — correctly — decided minutes earlier to leave the historical body's old paths to the concordance rather than rewrite them; that correct decision had quietly licensed leaving the *cold-start* sections stale too, even though those are the front door, not the historical record.

**The generalisation.** Verifying a document for its reader means reading it in the reader's order and from the reader's cold-start state, not in the author's editing order from the author's full-context state. "I changed it" and "it now works for someone who has only it" are separate claims needing separate checks — here, literally checking that every path in the cold-start list resolves. This sits one level up from the session's reorg principle (history-of-record resolves via concordance; the entry point must be current *in place*) and one level up again from Entry 29's α lesson (a value-level check is blind to meaning): each is a case of *the check I ran not being the check the claim needed*.
