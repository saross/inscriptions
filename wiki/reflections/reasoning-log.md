...
## 2026-06-11 → 2026-06-14 — Entry 16: in-stream notes from the cc-library adoption → refit → robustness → amendment → lodgement arc

*A robustness check earns its keep by failing informatively.* The global-θ hybrid was too weakly identified to be the lodged robustness vehicle (convergence *worse* with more compute, prior-invariant — an α↔θ_gen ridge). But its failure *surfaced* θ_gen ≈ 0.024, far from the calibrated 0.155 — which the re-derivation (0.025, 2.5× better fit) and the θ-sweep (27/29 stable) then confirmed. A check that cannot return "the thing you're varying is broken in a way you didn't expect" is theatre; this one could, and did — even from inside a model I'd already decided not to trust for its intervals.

*Empirical-Bayes plug-in from a biased first pass is circular.* `calibrate_theta` fit θ_gen as the intercept of `aligned_frac ~ θ` over the *shared-basis* (under-attributing) α's, inflating it (0.155). Re-derived from the *corrected* cc α's → 0.025, RMSE 0.045 vs 0.117. The bias in the inputs propagated into the calibrated parameter and back into production, and nothing internal flagged it. When a plug-in calibration uses an estimate the model will later correct, re-derive it from the corrected estimate and check it didn't move.

*The person who did the run is the most likely to mix runs.* I updated the amendment's frontier α's to the adopted-θ refit but left empire-aggregate's R̂/ESS at the stale first-pass values — the exact run-mixing I'd flagged as the risk all session. An independent verification agent re-reading the source caught it. Verify-at-source is not optional *because* you were careful; a confident mental model of "which number is current" is precisely what glosses the stale one.

*Confirm the downstream spec before lodging a claim about it.* I nearly lodged §A5.6 framing exploratory H3b as a "confirmatory family" and freezing a 26-unit eligible set via a criterion that conflicted with the H3b draft's *own* unsettled one (gap-rule 17 vs swing-rule 9). Reading the actual spec — H3b is exploratory, consumes the median SPA, deliberately propagates no uncertainty — reframed the section entirely. Read the text you're writing *about*.

*Propagate the uncertainty to dissolve an arbitrary upstream threshold.* The H3b eligibility cutoff (θ-range < 0.10) was a round number I picked. The principled fix (decision #1) was not a better threshold but to propagate the deconvolution posterior into the deviation test, so weak identification → wider result automatically — which makes the identifiable-set criterion moot. When a gate needs an arbitrary threshold, ask whether the uncertainty it is proxying can simply be propagated instead.

*Read the target before deleting it — a gitignore can carry intent.* The 2026-05-26 grid's gitignore deliberately re-included one smoke-cell fixture as a committed "research record" — but the re-include was half-finished (summary committed, raw replicates not). A blanket "synthetic cells are regenerable → remove" rule would have wiped a deliberately-kept fixture. Read the comment and the tracked-file state before acting on a retention rule.

*Memo to future me:* the reflexes that paid off — **let a failed diagnostic tell you what it surfaced, not just that it failed; re-derive a plug-in calibration from the corrected estimates; run an independent source re-read before any lodgement (you are the most likely to have mixed runs); read the downstream spec before lodging a claim about it; propagate uncertainty rather than pick a gating threshold; read what a deletion target actually is before deleting it.**

## 2026-06-14 → 2026-06-15 — Entry 17: in-stream notes from the /tmp-hygiene → H3b draw-wise base-run arc

*A stale record's *actionable list* is the trap, not its vagueness.* The continuity handed me four named "offending scripts" to fix for the `/tmp` leak — specific enough to just do. Three didn't exist; the real leak was a 201 GB `~/.pytensor/numba` cache the record never mentioned. The diagnostic was `df -i` + `du ~/.pytensor`, not the list. A confident, concrete, wrong pointer costs more than a vague one because it invites execution.

*Build the disambiguator before you need it.* The Stage-B engine carried a self-test (inlined envelope == library, bit-for-bit). When the result came back p=0 for all 29 units — every instinct screaming "bug" — the self-test had already excluded the engine, so the saturation resolved in one read to "the method is degenerate here," a finding. Without it I'd have spent the night hunting a non-bug. Construct the test that separates "broken code" from "broken result" *while you still expect the result to be clean*.

*The survey-first rule covers a prior run's code, not just shared tooling.* I wrote a fresh draw-wise engine and framed OQ-5 (the null construction) for Shawn — and only then, when Write blocked me, found the existing `h3b_lib.py`, already run, already reporting the saturation, already fitting CPL to the *observed* curve (the right choice). Reading it first would have reframed OQ-5 and surfaced the wall a decision-cycle earlier. Cost: a confirmed-then-corrected decision (OQ-5 → D1).

*A featureless null must be fit to the curve under test, not its precursor.* CPL-fit-to-raw-corpus makes the entire convention-removal reshaping read as deviation and saturates the probes; CPL-fit-to-observed-corrected tracks the curve's own trend so only *local* departures show. Same machinery, opposite informativeness — the fit target is the load-bearing choice.

*A preregistered test validated on a matching baseline can be degenerate on real data.* Phase-1 calibrated detection at N≈1,600 by injecting an event onto the *generating* smooth shape. On real data the null can't reproduce the true shape, so misfit + large N saturates the global test in a regime the calibration never probed. Validation-on-synthetic does not certify behaviour-on-real when the real null is misspecified.

*At a genuine methodological fork, overnight: frame, don't decide.* Hit global saturation + the CPL fit-target + the large-N question at 2 am; wrote a decision note with the diagnosis and three framed choices instead of choosing. Morning D1–D3 took minutes. The night's job was to make the human's decision cheap, not to pre-empt it.

*Size a follow-up by searching, not asserting.* Quoted baorista as "~1 week, greenfield"; Shawn's "didn't we build some of that?" → infra installed + smoke-tested since 2026-05-03 (`runs/2026-05-03-baorista-install/`), so ~2–4 days. A confident estimate stated without a `grep` is the same failure mode as the stale continuity, authored by me.

*Memo to future me:* **distrust a stale record's most actionable line hardest; build the broken-code-vs-broken-result test before the run; read the prior run's code before re-framing its open questions; fit a null to the curve under test; don't trust synthetic validation to certify real-data behaviour when the null is misspecified; frame-don't-decide at an overnight fork; and `grep` before you size a follow-up.**

## 2026-06-15 → 2026-06-16 — Entry 18: in-stream notes from the flexnull-annex → deconvolution-leverage → §5-batch arc

*Ask whether the lever moves the answer before you spend compute on it.* Four times a cheap diagnostic pre-empted an expensive run: does α correlate with population (→ no, so the deconvolution can't move the cumulative scaling); does the peak-shift scale with size (→ no, so it can't move the peak scaling); does the balanced width pool even differ from global (→ check before a 540-cell grid re-run). The question "would this change the result?" is almost always cheaper to answer than the result.

*Re-read the spec before implementing a preregistered model; the *form* is load-bearing, not just the parameters.* I built D11's measurement-error layer as a structural errors-in-variables hyperprior — reasonable, but the prereg specifies `log_pop_c ~ Normal(log_pop_observed_c, σ_pop)`, the Berkson form. The σ grid was right; the model structure was wrong. Killed the launched run and redid it. "Preregistered" means the structure is specified, not just the knobs.

*Read the harness before you swap a component out of it — the obligation may be moot.* B4 stratifies the Phase-1 bootstrap; reading `h1_sim_v2.py` showed Decision 8 had replaced that bootstrap with synthetic-data-from-null, making B4-as-written architecturally moot and its province/city counts vestigial. An obligation written against a superseded design needs reinterpretation, not execution. The tell was in the harness, not the prereg.

*In a log-space slope, a near-zero observation is a high-leverage outlier; reach for a robust estimator before you believe it.* OLS slope of log α on log pop = +0.29 (material); Theil-Sen = −0.03, and dropping the one α≈0.016 unit (Pompeii) collapses OLS to +0.015. The headline was a single point in log space; robust-estimator-plus-leave-one-out is the cheap guard.

*Know whether your statistic is mass-conserved or shape-sensitive before predicting how a reshaping affects it.* The deconvolution conserves each unit's full-window count (so it can't move the cumulative Hanson scaling) but reshapes the curve (so it moves the *peak* by ~60%). Same operation, opposite effect on two statistics — "integral over the window" vs "functional of the shape." Identify which you have first.

*Run the baseline at the same reduced precision as the treatment so precision cancels in the delta.* The B4 threshold re-run used n_iter=200, not the committed 1000; comparing reduced-balanced to full-precision-global would confound precision with the width effect. Running global at matched reduced precision makes the balanced-vs-global Δ clean even though the absolute thresholds are noisy.
...
## 2026-06-17 → 2026-06-18 — Entry 20: in-stream notes from the residual-Layer-B → size-vs-dynamics → province-size → audit-refresh → Latin-variants arc

*When a pre-specified summary statistic contradicts the mechanism you understand, suspect the statistic and inspect the raw object.* The residual Layer B's "frac-of-peak at AD 250" came back ≈ 0, mimicking the raw collapse I'd predicted it would dissolve. Rather than report the surprise, I plotted `q` vs the empire baseline — median 0.32, not 0. The metric was confounded; the mechanism was right. Look at the trajectories, not the one number.

*The right diagnostic for a geom-mean-1 (zero-sum-log) quantity is "vs the baseline 1.0", not "fraction of own peak".* Peak-normalising an `exp((1/β)·residual)` trajectory is dominated by the GRW's endpoint variance (a third of cities "peak" at an envelope edge) amplified by `1/β>1`, forcing the ratio to ~0 regardless of the late level. Frac-of-peak was the wrong yardstick for an amplified random walk; demote it to a flagged footnote.

*A monotone, group-constant transform preserves cross-group ranks — so you can skip a frame you don't need.* The residual inversion's `1/β` rescales every city's trajectory by the same per-draw constant, so Spearman ρ (city-feature vs size) is **exactly β-frame-invariant per draw**. Empire β and Latin β give identical rank correlations; I ran only empire and noted Latin carries over. Saved a redundant fit.

*Launch a follow-up that can disconfirm a just-made claim, and distinguish a decomposition fact from a mechanism.* Obs 104 inferred "province-mediated" from `q_uv ≫ q_v`; the direct province-size regression did not corroborate it. The variance living in the `u` tier (a decomposition fact) is **not** the same as province size driving the trajectory (a mechanism) — the regression separates them, and the honest reading is the weaker one. Pre-frame the null as informative *before* seeing it (n too small), so a null isn't a disappointment.

*Re-verify a status claim at its authority before stating it — not from a stale audit's status column.* I told Shawn "Amendment 02 pending" from the 2026-06-05 audit; the git tags showed all four amendments lodged (A02 since 2026-06-06). Lodgement authority = the tags + summary-addenda, not a fortnight-old planning doc. The read-side anti-confab rule applies to spoken *status* claims, not just filenames/hashes.

*Verify a column/config name at the source before it enters a spec.* The Hanson population field wasn't `urban_context_pop_est` (a stale name carried in an old spec); the project reads `pop_est` from `city-index.parquet`. A two-line read of the CSV caught it before it propagated into the size-vs-dynamics design.

*Bake an exact algebraic self-check into a transform script as a wiring guard.* The residual Layer B asserts that adding `g` back reproduces the raw Layer B `shape_med` (within-city levels cancel under peak-normalisation) — it passed at 5.6e-16; the `q_u` extension asserts `q_uv = q_u·q_v` per draw — passed. When a transform has an exact identity, make the script fail loudly if it breaks; a near-free regression guard that proves "the only change is X".

*Background agents: brief them to do everything EXCEPT git; centralise commits in the parent.* Two background agents (province-size, Latin variants) ran while the main thread worked; with the parent owning all commits, no concurrent-push race, and the agents leave artefacts for verify-at-source-then-commit. Re-check the agent's headline numbers against its own `summary.json` on return regardless of how clean the report reads.

*Memo to future me:* **when a summary statistic fights the mechanism, inspect the raw object; use "vs baseline 1.0" not "frac-of-peak" for a zero-sum-log/amplified quantity; rank stats are β-frame-invariant under a monotone group-constant transform (skip the redundant frame); test a just-made claim with a follow-up that can disconfirm it and split decomposition-fact from mechanism; re-verify a status claim at its authority (git tags for lodgement) not a stale audit; verify a column name at source before it enters a spec; bake exact algebraic identities into transform scripts as wiring guards; and run background agents git-free with commits centralised in the parent.**

## 2026-06-18 → 2026-06-19 — Entry 21: in-stream notes from the supplementary-wave → H9 → C10-validity → C10-(ii) → lit-scout orchestration session

*The audit-before-run gate earns its cost, and the friction concentrates in what you over-spec.* The H2.1 supplementary driver took three audit rounds — every defect in optional add-ons I'd specced past the obligation (a cross-family IC; a C11 output-level r); the prereg-required core (α side-by-side, dispersion, input-level C11 r, H2.2/2.3/2.4, C16) was clean from audit 1. Each caught bug was a *silent* failure (a gate asserting bit-identity on sampled draws → always-fail; a cross-family PSIS-LOO that's undefined → silent `?`; `az.waic` that doesn't exist → uniform error; the wrong ELPDData accessor → uniform NaN). Gate the run; don't gold-plate past the prereg.

*Instrument the factors, not your guess about them.* I bet the C10 collapse mechanism wrong twice (reading-(b) "signal destruction", then width R1); the realism-graded test (R0/R1/R2/R3/R1+R2) found θ-contamination (R2). The design that tests the dimensions, not the favoured hypothesis, caught it.

*A pre-registered binary can be technically satisfied while missing the substance.* The C10 validity rule returned "(a) — C10 stands" (point-date recovers planted α on synthetic), which alone would have closed the question; the real finding was the 1b/1c mismatch (synthetic recovers, real collapses). Flag the tension and build the follow-up; don't bank a clean verdict that contradicts a real-data observation.

*When a tool fails repeatedly but a minimal probe of the same tool succeeds, the command is self-sabotaging — check that before blaming infra.* Repeated SSH `255`s I read as rate-limiting were `pkill -f run_c10_ii` matching its own wrapper shell; `ssh -vvv echo` succeeded (no self-matching pattern). The `[r]un_c10_ii` trick fixes it — and the same self-match (a `pgrep` matching its wrapper) is why an earlier completion-Monitor never fired.

*Verify a library's API against the installed version, not memory.* arviz 1.x removed `az.waic` (use `az.loo`); its `ELPDData` exposes `.elpd`/`.p`, not `.elpd_loo`/`.p_loo` (the repr row-labels are not members). Confirm empirically (`hasattr`, a one-cell call) before relying on an accessor.

*Cross-model information criteria need a common observation footing.* PSIS-LOO across a joint-multinomial (≈3 pointwise points/unit) and a per-bin NegBin (≈161) is undefined; `az.compare` can't rank them. For "reported-alongside" model-comparison, use the dispersion adjudicator + α side-by-side, not a forced IC.

*Parallelise independent-cell sweeps by default; it's the project norm.* The C10 scripts ran sequentially (a spec oversight — ~5 h vs ~1 h); wrapping the cell loop in the proven supp-wave `ProcessPoolExecutor`/spawn pattern (cells independent, seeds deterministic, results slotted by index) cut it. Standardise into a reusable sapphire-parallel-run module (slated).

*Feed the canonical Zotero importer; don't drag-drop.* It re-fetches metadata per DOI and dedups across group + user libraries (caught 8 of 21 already held across SDAM-AU/TRAP/My Library — a raw `.bib` drop would have duplicated them). A single-pass `/lit-scout` doesn't emit the workspace it needs, but you can assemble one from the verified claims + report + corrections and dry-run before `--live`.

*Re-read the committed result at source before reporting it; an agent's summary is a pointer.* Every run verdict (supp-wave, C10, (ii)) was re-extracted from the committed JSON/report before relaying.

*Memo to future me:* **gate every run but don't over-spec the add-ons (the prereg core was clean; the extras caused all the churn); instrument the factors not your hypothesis (θ, not width); don't bank a pre-registered binary that fights a real-data observation; a repeatedly-failing tool whose minimal probe succeeds is self-sabotaging (pkill/pgrep self-match → use `[r]`); verify library APIs against the installed version (az.loo not az.waic; ELPDData.elpd not .elpd_loo); cross-family IC needs a common observation footing; parallelise independent sweeps by default; feed + dry-run the canonical Zotero importer (dedups across all libraries); and re-read committed results at source before reporting.**

## 2026-06-19 → 2026-06-20 — Entry 22: in-stream notes from the analytical-close → documentation-audit → accuracy-workflow → collaborator-summary → figures-spec arc

- **D13 first, prereg-literal, honest about power.** Chose Option A (standalone per-city α at N≥100) over the hierarchical B′ because B′'s shrinkage would suppress the very within-province α-variation the test targets, and the MI propagation makes A's low-N unreliability *visible* rather than hidden. Result a clean null; B′ then had no scientific case (parked).
- **H4 closed by adopting q_u; H6/B′ deferred.** Recognised q_u (province-from-empire) already *is* the province-scale Layer B for this paper (validated-by-inheritance), and that a standalone province inversion would need a province-level exponent + has no validation anchor → follow-up. Kept the paper's scope from creeping.
- **Audit-then-build sequencing + single-committer agents.** Ran read-only assurance sweeps before any edits; used *one* sequential cleanup agent (not parallel) specifically to avoid concurrent-push races on `main`. The git-hygiene discipline (re-verify 0-behind, explicit pathspecs) held across ~20 commits and several background agents.
- **Option 1 on the lodged ~31%.** The lodged amendment stands unedited (it is the timestamped record); the paper cites the reproducing figures (~36% F1+F3 / 24.96% full pool) with a footnote. Don't retro-edit lodged records; correct forward.
- **Workflow design.** Cluster verifiers (fan-out) → adversarial per-mismatch re-verify (kill false positives) → synthesis with a cross-document-consistency scan; schema-forced structured output so results were machine-mergeable. Expanded scope (figures + paper-facing docs + cross-consistency) by stop-and-relaunch rather than half-measures.
- **"The file is wrong" → verify at source first.** Shawn saw v1 in VS Code; resisted assuming a git/sync fault — confirmed disk + origin + all machines held v2, diagnosed a stale editor buffer. The discipline: a discrepancy report is a hypothesis, not a diagnosis.
- **Figures: spec-then-build (option b).** At ~60% context, chose to freeze every design + data decision into a spec now (when I held the verified numbers + data locations) rather than half-produce 13 figures and rush the ones that matter. Claude Design ruled out for data figures.
- **Refinements (3-way temporal split; verbosity).** The 54% empire-common is sourced; the province/city sub-split (≈24/≈22) is apportioned by near-equal component variances and flagged indicative (exact split deferred to the idata regen). Verbosity check: idiosyncratic — not capitals/size/output — which *reinforces* (rather than merely sits beside) the orthogonality finding.

---

## 2026-06-20 → 2026-06-21 — Entry 23: in-stream notes from the figures-build → α-inversion → Roma/Italia → women-Option-2 autonomous block

- **No regen needed — checked first.** The figures spec said the §5 `.nc` had to be regenerated on sapphire (~45–80 min). Before launching, I checked whether the original existed — it did, intact on both sapphire and zbook (md5-verified) — and copied it down. The spec's "must regen" was an assumption; verifying it saved the whole compute step. *Default to checking whether the artefact already exists before reproducing it.*
- **The temporal-split refinement turned out non-trivial.** Last session's indicative ≈54/24/22 assumed the three tiers were independent. The exact computation showed they are *anti-correlated* (marginal shares sum to 131%; covariance −31%). Reported all three candidate partitions; Shawn chose the clean covariance-attributed one (38/29/33) with the 54% standalone footnoted. The lesson: "indicative, pending exact" can hide a structural feature, not just a precision gap.
- **α-inversion: triangulate before flipping a headline.** Suspicious women α≈0.90 → checked the model (`p_mix = α·p_conv + (1−α)·p_gen` ⇒ α=convention) → confirmed by the pi_align identity → made certain by Pompeii (α=0.016, must be ~all genuine ⇒ α=convention). The whole canonical corpus (prereg, model, amendments) already had it right; only the collaborator summary was inverted. Corrected forward (banner + body), flagged the headline framing for Shawn — who confirmed it (he'd expected ~⅔ convention from the ~100y median range).
- **Verify-before-build voided decision-1.** Almost coded the Roma driver on `h2_lib.fit_unit` (superseded). The production model is `refit_lib` + `joint_lib` cross-classified *library* — one universal slab basis for all units. That dissolved the "empire vs Latin basis" choice entirely: comparisons are clean by construction, units differ only in membership. A sign-off can be made moot by later source-reading.
- **Audit-before-launch + reuse fit_one verbatim.** New units fitted by importing the production `fit_one` (byte-comparable to the 29 production units), with fresh `unit_index` ≥100 for collision-free seeds, draws emitted to *this* run's outputs (no production pollution). A background code-review caught one real bug (`n_eff` vs `n_rows_eff`) before the run.
- **Women probe: a cheap dry-pass killed a wrong design.** Intended to test whether *narrow date intervals* give a reliably de-foggable subset. The dry-pass (N + aligned-fraction per width) showed width is the *wrong axis*: convention lives at *round* widths (F1_round 25y slabs = 68% of rows), so the ≤50y band is the *most* convention-dated (α 0.97). Fitted the width ladder anyway to report actual α — all > 0.70. The dry-pass saved fitting the wrong thing.
- **Three-option ladder: reconstructed, not recovered.** Shawn recalled a 3-way framing (do-now → middle → new-paper); the records only had the 2-way (vignette / EJA companion) — confirmed by searching memory, docs, *and* the transcripts (safely, `rg -z`). Reconstructed the middle rung from the gap-logic; Shawn confirmed it was right. When the record genuinely lost something, reconstruction-from-structure + human confirmation is the honest path (do not pretend to "recall" what is not there).
- **tempun as a complementary cross-check.** Installed `tempun` (Adela's group's package) + ran it ourselves. Its MC mean overlays the *raw* aoristic shape (both peak ~AD 188) — it models dating uncertainty but is blind to convention. Concrete proof that aoristic-MC and convention-de-fogging correct different artefacts.

## 2026-06-23 — Entry 24: in-stream notes from the JAMT-outline → repo-canonicalisation → FAIR4RS-scout+uplift → continuity-self-sufficiency session

- **Hard-to-reverse work gets a gate, even solo.** The repo reorg (~2,000 tracked files, OSF artefacts) ran read-only survey → written proposal (stopped at the gate) → branch + PR execution, per the repo-standardise spec's own human gate and the global branch-for-migrations rule. Did not auto-execute the moves while Shawn was away earlier; waited for his go-ahead + decisions D1–D7.
- **Directory boundaries are about how an artefact changes, not what it is about.** D1: a *lodged* prereg is a frozen registered record, not a mutable plan → `wiki/prereg/`, not `wiki/planning/`. The same logic kept the living bibliography in its own `sources/` rather than `planning/`.
- **Concordance, not rewrite, for history-of-record.** ~76 of 78 old-path references live in `runs/` (immutable run records); rewriting them would falsify the research record. Left them to the `wiki/index.md` concordance; rewrote only live docs (README, key-findings, active plans). The exception that proves the rule: continuity's *cold-start* sections are the front door, not history → those I did rewrite.
- **Run the validator; don't trust the file.** `cffconvert --validate` caught a real schema bug in my hand-authored CITATION.cff (reference `type: dataset` → must be `data`). Authoring + validating beats authoring + assuming.
- **Honest baseline over a chased badge.** howfairis scored 3/5; the two misses (package registry, OpenSSF checklist) are genuinely N/A for a research-*analysis* repo, so I documented *why* in the README rather than gaming the badge to 5/5.
- **"Updated" ≠ "self-sufficient."** Caught (via Shawn's question) that updating continuity's remaining-work + beacon left its cold-start orientation sections stale in content and paths. Audit a handoff doc in the *reader's* reading order, not the editor's — and verify it concretely (checked that every refreshed cold-start path resolves).
- **Sync discipline across three machines.** FF-pull zbook + sapphire to origin, re-pull after every subsequent commit, and a final all-four-at-the-same-SHA check — because the next three weeks run on zbook. Reported (not silently kept) the only untracked artefacts: tiny run-logs on sapphire, regenerable H9 parquets on zbook.
